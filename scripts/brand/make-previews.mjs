...
for (const { file, label } of CARDS) {
  await page.setContent(`<!doctype html><html><head><style>
    html,body{margin:0;width:800px;height:411px;background:#101617;overflow:hidden}
    .glow{position:absolute;inset:0;background:radial-gradient(ellipse at 50% 42%, #59cfae24 0%, transparent 70%)}
    .wrap{position:absolute;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:26px}
    img{height:150px}
    .label{color:#93a8a3;font:600 26px/1 Georgia, 'Times New Roman', serif;letter-spacing:0.35em;text-transform:uppercase;margin-left:0.35em}
    .rule{width:64px;height:2px;background:#59cfae;border-radius:1px}
  </style></head><body>
    <div class="glow"></div>
    <div class="wrap">
      <img src="${b64}">
